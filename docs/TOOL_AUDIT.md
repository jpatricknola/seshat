...
| ~~**Quantize notes** (`quantize_clip`)~~ · **ADDRESSED 07/2026**          | ~~Medium~~ | `quantize_clip` calls Live's own `Clip.quantize` through the fork's existing `/live/clip/quantize`, so the grid arithmetic never gets reimplemented in Elixir — which turned out to matter: the documented `GridQuantization` table was wrong in every row, and the tool's string enum (`"1/16"`, `"1/8T"`, …) keeps the corrected integers behind one private function. Still out: per-pitch quantize (`Clip.quantize_pitch`), audio-clip warp-marker quantize. (Groove and swing amount landed separately — see the row below.) |
| ~~**Set time signature** (`set_time_signature`)~~ · **ADDRESSED 07/2026** | ~~Low-Med~~ | `set_time_signature` completes the tempo pair on two upstream addresses. The schema is what makes it safe — numerator 1–99, denominator an enum of Live's own {1, 2, 4, 8, 16} — because `_set_property` swallows a Live API rejection and setters never reply, so an invalid value would be a silent no-op. Still out: per-scene signature overrides (`/live/scene/set/time_signature_*`). |
...
| ~~**Swing / groove amount**~~ · **ADDRESSED 07/2026**                     | ~~Low-Med~~ | `set_swing_amount` and `set_groove_amount`, both mirrored into `get_session_state`. Two knobs rather than one because they are different Live concepts: swing needs a follow-up `quantize_clip` to be heard, groove needs a groove assigned to the clip by hand — which Seshat cannot do (`Clip.groove` is an unserializable LOM object), so its description says so instead of promising swing. `swing_amount` is a fork addition (one `properties_rw` line in `song.py`); `groove_amount` is upstream's, re-bounded to Live's real 0.0–1.3. Still out: assigning a groove, loading the Groove Pool, per-clip swing (no LOM surface), `midi_recording_quantization`. |
| **Groups · routing/IO · automation**                                      | Low      | Breadth for later — grouping tracks, input/output routing & monitoring, automation envelopes.                                                       |
...
| `set_tempo`             | Transport | Keep   | Pairs with `set_time_signature`.                         |
| `set_time_signature`    | Transport | Keep   | New 07/2026. Two silent upstream setters; validity is enforced by the schema (numerator 1–99, denominator enum {1, 2, 4, 8, 16}) since a rejected value looks exactly like success on the wire. Reply states the bar length in quarter-note beats, which is what clip lengths and note times keep counting. |
| `set_swing_amount`      | Transport | Keep   | New 07/2026. The "make it swing" knob for plain MIDI, on a fork-added address (`swing_amount` in `song.py`'s `properties_rw`). Silent and fork-only, so the reply carries the `mix abletonosc.install` hint; the description states that swing is only heard once `quantize_clip` runs. Mirrored, so `get_session_state` is the read side. |
| `set_groove_amount`     | Transport | Keep   | New 07/2026. Upstream's Groove Pool Amount dial, bounded 0.0–**1.3** against Live's own Move script rather than the apiref's understated 1.0. Its description's real job is routing: it only scales grooves the user assigned by hand, so it points at `set_swing_amount` for plain MIDI. Mirrored like swing. |
