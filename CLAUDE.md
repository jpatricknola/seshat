...
- To exercise the real loop you need Ableton Live running with AbletonOSC installed. See [README.md](README.md). Three pieces divide that work, and keeping them separate is what stops the live layer's checks from accreting into one unrunnable file (which is exactly what happened before 2026-08-02):
  - **`/write-smoke-tests`** derives the checks a change needs and writes them into its plan doc's `## Live verification` section. It owns the rules — which properties of a change (a vendored address, a send-only setter, a new listener, model-facing text, an unmeasured mapping) imply which checks — and the format. Needs no Ableton; `/plan` calls it so the verification is written while the reasoning is fresh and lands in front of `/plan-review`.
  - **`/smoke-test`** runs them against live Ableton and reports honestly. It does not invent checks: if a plan has no `## Live verification` section, it stops and runs `/write-smoke-tests` first.
  - **[docs/live-invariants.md](docs/live-invariants.md)** holds the checks that outlive any one feature — standing properties, tripwires guarding a corrected measurement, and model-behaviour probes. Entry is by promotion at `/ship`, never by writing there directly; everything else archives with its plan. Each section carries a `Last verified` date, which is the only record anywhere that the live layer was actually exercised.
- `/full-smoke` sweeps every section of `live-invariants.md` in one pass, scoped to what needs no human: no Live restart, no server restart, no ears, no second client. It defers to `live-invariants.md` for what each check *is* and to `/smoke-test` for how to judge one, deciding only what runs, what gets an automated stand-in, and what is skipped — so its report always carries a named Uncovered list. Use it as the routine sweep; use `/smoke-test` when a specific change needs its own plan's checks run too.
